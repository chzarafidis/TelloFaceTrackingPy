...
if __name__ == '__main__':
    ...
    pan_pid = PID(kP=0.6, kI=0.0001, kD=0.1)
    tilt_pid = PID(kP=0.6, kI=0.0001, kD=0.1)
    pan_pid.initialize()
    tilt_pid.initialize()
    ...
    while True:
        ...
        if rect is not None:
            ...
            if run_pid:
                ...
                if track_face and fly:
                    # left/right: -100/100
                    lr_v = int(pan_update // 3)
                    ud_v = int(tilt_update // 3)
                    if lr_v != 0 or ud_v != 0:
                        tello.send_rc_control(lr_v, 0, ud_v, 0)
                    else:
        ...
        if frameCount == 0 :
            #cv2.moveWindow("Gray - Drone Face Tracking", 200,200)
            cv2.moveWindow("Color - Drone Face Tracking", 300,300)
